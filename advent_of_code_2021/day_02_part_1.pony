...
class Day02Part1
  ...
  =>
    ...
    let result_collector = _Day02Part1ResultCollector(err, notify)
        ...
        {(result, value) =>
          ...
          | let success: Success =>
            ...
            | let command: Command =>
              ...
              err.print("Day02Part1: invalid command at line " + (i+1).string()
                + " '" + line' + "'")
            end
          | let failure: Failure =>
            err.print("Day02Part1: failed to parse line " + (i+1).string()
...
actor _Day02Part1ResultCollector
  let _err: OutStream
      ...
      if _results.size() == n then
        ...
        for i in Range[USize](0, n) do
          try
            ...
            | Forward => horiz = horiz + command.distance
            | Down => depth = depth + command.distance
            | Up => depth = depth - command.distance
